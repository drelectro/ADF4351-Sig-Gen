/*!
   ...
   Mike Cornelius (VK2XMC), 2025, Port to support AliExpress ADF4351 synthesizer module
...
//            MOSI  MISO  SCLK   SSEL
SPIClass ADF_SPI(ADF4351_MOSI, ADF4351_MISO, ADF4351_SCK);
...
int  ADF4351::setf(uint32_t freq, uint16_t phase, uint32_t chan_steps)
{
  ...
  if ( Mod < 2 || Mod > 4095) {
    Serial1.println(F("Mod out of range")) ;
    return 1 ;
  }

  if ( (uint32_t) Frac > (Mod - 1) ) {
    Serial1.println(F("Frac out of range")) ;
    return 1 ;
  }

  if ( Prescaler == 0 && ( N_Int < 23  || N_Int > 65535)) {
    Serial1.println(F("N_Int out of range")) ;
    return 1;
...
int  ADF4351::optimise_f_only(uint32_t freq, bool debug,bool log_info, bool gcd_method)
{
  ...
  if(gcd_method==true){
    ...
    if(setf_only(freq,s,debug)==0){
      if(log_info==true){
        Serial1.print("Common divisor: ");
        Serial1.println(s);
        Serial1.print("Step Frequency set to: ");
  ...
  if(freq_set==false && log_info==true){
      Serial1.println("Frequency not set");
  }
  return 0;
}
...
int  ADF4351::setf_only(uint32_t freq, uint32_t chan_steps, bool debug)
{
  ...
  if ( freq > ADF_FREQ_MAX ) {
    if(debug){
      Serial1.println(F("Freq out of range (HIGH)")) ;
    }
    return 1 ;
  }
  ...
  if ( freq < ADF_FREQ_MIN ) {
    if(debug){
      Serial1.println(F("Freq out of range (LOW)")) ;
    }
    return 1 ;
  }
  ...
  if(debug){
    ...
    Serial1.print(freq) ;
    Serial1.print(F(" N_Int: ")) ;
    Serial1.print(N_Int) ;
    Serial1.print(F(" Mod: ")) ;
    Serial1.print(Mod) ;
    Serial1.print(F(" ChanStep: ")) ;
    Serial1.println(ChanStep) ;
  }
  ...
  if ( cfreq != freq ) {
    if(debug){
      Serial1.println(F("output freq diff than requested")) ;
    }
  }
  ...
  if ( Mod < 2 || Mod > 4095) {
    if(debug){
      Serial1.print(F("Mod out of range: ")) ;
      Serial1.println(Mod) ;
    }
    return 1 ;
  }
...
int ADF4351::writeRegisters(bool debug)
{
  int i;
  if(debug){
    Serial1.println("writing to ADF") ;
  }
  for (i = 5 ; i > -1 ; i--) {
    writeDev(R[i]) ;
  ...
  if(debug){
    Serial1.println("Written to ADF") ;
  }
...
double ADF4351::setPhaseAngle(double phaseAngle)
{
   if(phaseAngle>360 || phaseAngle<0){
    Serial1.println("Phase Angle range is 0-360");
...
{ 
  if(pwrlevel>3){
    Serial1.println("Amplitude range is 0-3");
    pwrlevel=3;
  } else if(pwrlevel<0){
    Serial1.println("Amplitude range is 0-3");
    pwrlevel=0;
...
void ADF4351::writeDev(Reg r)
{
  ...
  for ( i = 3 ; i > -1 ; i--) {
    ...
    //Serial1.println("writeDev Transfer") ;
    ADF_SPI.transfer(txbyte) ;
  }
  
  //ADF_SPI.transfer(&(r.whole), 4, SPI_TRANSMITONLY);
  digitalWrite(pinSS, HIGH) ;
  delayMicroseconds(1) ;
  digitalWrite(pinSS, LOW) ;
  //Serial1.println("writeDev Complete") ;
}


  void ADF4351::freqInfo(){
    Serial1.print("Freq:");
    Serial1.println(cfreq) ;
    Serial1.print("PLL INT:");
    Serial1.println(N_Int);
    Serial1.print("PLL FRAC:");
    Serial1.println(Frac);
    Serial1.print("PLL MOD:");
    Serial1.println(Mod);
    Serial1.print("PLL PFD:");
    Serial1.println(PFDFreq);
    Serial1.print("PLL output divider:");
    Serial1.println(outdiv);
    Serial1.print("PLL prescaler:");
    Serial1.println(Prescaler);
    //Serial1.print("Lock Detect:");
    //Serial1.println(digitalRead(PIN_LD));
    Serial1.print("RF Enable:");
